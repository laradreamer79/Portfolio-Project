...
| **Database** | MongoDB (Mongoose) | Stores all application data: users, diving centers, trips, courses, bookings, and reviews. Uses document-based collections. |
...
4. The backend queries **MongoDB** using Mongoose to retrieve or store data.
5. MongoDB returns the result to the backend, which formats it as a **JSON response**.
...
8. When map data is needed, the frontend calls the **Google Maps API** directly using the center's coordinates stored in MongoDB.
...
|-------------|-------------|
| **Development** | Local machines — each developer runs the full stack locally using Node.js and MongoDB Atlas |
...
| **MongoDB** | Database | Oyster's data (diving centers, trips, courses) is semi-structured and varies across centers. A document-based NoSQL database provides the flexibility needed, unlike a rigid relational schema. MongoDB Atlas offers a free cloud tier suitable for the MVP. |
...
| **MongoDB Atlas** | Cloud Database | Free tier available, no infrastructure management required, built-in backups and monitoring. Ideal for an MVP with a student team. |
...
| **Scalability** | MongoDB scales horizontally. Node.js handles concurrent requests efficiently with its non-blocking I/O model. |
